...
export class AnularMultaComponent implements OnInit {
  ...
  goBackToMultas() {
    setTimeout(() => {
      switch (this.error) {
        case 'Notificacion Anulada Correctamente':
          location.reload();
  ...
  anularMulta() {
    let post = {
      p_correl: Number(this.idcorrl),
      p_resanu: this.resanu,
      p_fecanu: this.fecanu,
    };
    console.log(post);
    this.spinner.show();

    this.sigtaService.anularMulta(post).subscribe({
      next: (data: any) => {
        this.spinner.hide();
        console.log(data);
